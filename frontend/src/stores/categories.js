  ...
  actions: {
    async fetchCategories(force=false) {   
      ...
      try {
        if (force || this.categories[RTYPES.expense].length == 0) {
          const expenseResponse = await CategoriesAPI.getCategories(RTYPES.expense);
          this.categories[RTYPES.expense] = expenseResponse.data;
        }
        if (force || this.categories[RTYPES.income].length == 0) {
          const incomeResponse = await CategoriesAPI.getCategories(RTYPES.income);
    ...
    async updateCategory(type, category) {
      ...
      try {
        ...
        const idx = this.categories[type].findIndex((c) => c.id === category.id);
        if (idx !== -1) {
          this.categories[type][idx] = { ...this.categories[type][idx], ...data };
        }
      } catch (error) {
        this.error =
